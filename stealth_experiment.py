"""Tasks 2-4 of the stealth-geometry extension (CLAUDE2.md): the experiment that decides
the Section-V question, plus the Proposition-1 slack check against the EXISTING (8)/(9)
equilibrium attack. Reuses stealth_geometry.py (Task 1) and deepc.py/game.py's matrices;
does NOT modify the equilibrium reproduction.

--- Task 2: does the attack's physical impact depend on D through dist(D,R) alone? ------
Fix g = g_star (nominal tracking, eq. 5) and the true disturbance D on a ray c*d_hat,
c in [0,Dbar]. The attacker plays the geometrically-stealthy attack A that maximizes the
physical impact within the Prop.-1 ball ||FAbar(A-A_c(D))|| <= rho(D). Only the scalar
actuation component A0 = e1^T A moves the REAL applied input (the deception component A_r
only fakes y_ini, which is measured history -- it does not change the true plant trajectory;
same convention as game.py/sweep_budget.py), so impact is defined exactly as in
sweep_budget.py: push the worst-case constant actuation A0 through the true double
integrator and take the max |position| deviation.

Maximizing e1^T A over the ellipsoid (A-A_c)' M (A-A_c) <= rho^2, M=FAbar'FAbar (SPD, full
column rank) is a linear-objective-over-ellipsoid problem with the closed-form maximizer
(Cauchy-Schwarz, tight)
        A0_max(D) = e1^T A_c(D) + rho(D) * sqrt(e1^T M^{-1} e1) .
No iterative solver needed; this is exact.

Two families of D (unit direction d_hat, scaled by c):
  A1  d_hat = FAbar[:,0]/||.||  = [1_{n1};0_{n2};1_{n3}]/||.||   (constant actuation bias;
      exactly the direction of FAbar's own column 0, so dist(D,R)=0 identically)
  A2  d_hat = (0; w/||w||; 0), w random in R^{n2}                 (pure sensor bias; Fa2
      spans the whole sensor block, so this too has dist(D,R)=0 identically)
  B   d_hat in R^perp, D2=0 and (D1,D3) zero-mean (orthogonal to FAbar's column-0 direction)

Prediction: family A/A2 -> rho(D)=Dbar constant -> impact flat in c. Family B ->
rho(D)=sqrt(Dbar^2-c^2) -> impact shrinks to 0 as c -> Dbar.

--- Task 3: does the EXISTING (8) equilibrium attack respect the geometric bound? -------
game.py's attacker_response(d, g_star) is D-INDEPENDENT (its stealth ball is ||g*-g~||<=
delta_g on g, not tied to any true disturbance D) -- flagged in the CLAUDE2.md audit. So
there is no single "true D" the existing pipeline was solved against; instead we ask the
sharper question the mismatch raises: for VARIOUS candidate true disturbances D (spanning
families A/B at full budget c=Dbar, i.e. the hardest case, plus a few random ones with
dist(D,R)<=Dbar), does the existing pipeline's fixed attack A_eq=[A0;A_r] satisfy the
geometric necessary condition ||FAbar(A_eq-A_c(D))|| <= rho(D) + tol? A negative slack
means the existing model's attack is not actually stealthy against that D -- a genuine
model-mismatch finding, not a numerical artifact.
"""
